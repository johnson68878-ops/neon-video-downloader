...
- `neon/ui.py`：NEON MeetStudio主視窗、錄屏設定、開發者頁及錄製狀態控制。
- `neon/download_ui.py`：原下載介面，改為嵌入式繁體中文頁面。
- `neon/recorder.py`：FFmpeg gdigrab + PyAudioWPatch，後臺錄製、暫停片段合併、故障保留。
- `assets/`：應用圖示與 Windows 版本資訊。
- 釋出檔案為 `dist/NEON-MeetStudio.exe`。下方為 v1 下載模組的原始設計記錄。
...
| 顯示哪些平臺名稱、分享文字規則 | `neon/urls.py` |
| 畫質選項、檔名、重試、字幕、MP3 | `neon/downloader.py` |
| 解析資訊、下載結果或事件格式 | `neon/worker.py` |
| 新網站真正的解析能力 | 優先升級 yt-dlp；只加平臺名稱不會新增下載能力 |

## 程式與資料

- UI 在主程式，網路與轉換在 worker；Tk 元件只由 UI 主執行緒更新。
...
- 停止使用 Windows `taskkill /T` 終止這次 worker 及 FFmpeg 子程式。部分檔案保留供續傳。
- 「貼上即下載」預設開啟；先選解析度再貼。關閉後可先按「解析資訊」檢視來源提供的影片高度。
- 畫質是來源影片高度上限，不會把低畫質放大；無解析度資訊的來源仍可下載原檔。直式影片的高度可能是 1920，而非一般口語的「1080p」。
- 不讀取瀏覽器 Cookie、不繞過 DRM；網址裡的授權引數僅交給下載引擎，請勿把帶權杖網址提交到 Git。
