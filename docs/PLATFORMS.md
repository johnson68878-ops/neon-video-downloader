...
本程式使用 yt-dlp 的網站解析器，自動依網址選擇；不需手動選平臺。
...
| TikTok | tiktok.com、vm.tiktok.com、vt.tiktok.com | 與中國抖音不同平臺，由各自解析器處理 |
...
| Vimeo / Twitch | 影片或剪輯網址 | 依來源與許可權提供格式 |
| 其他 | yt-dlp 支援網站或公開影片直連 | 由通用或專用 extractor 嘗試解析 |

`neon/urls.py` 的平臺表只決定畫面標籤。實際支援由 yt-dlp 決定，沒有逐一實測上述平臺的線上下載。網站持續變更，名單不代表每支影片當下都能成功。
...
4. 若想先檢視影片資訊，取消「貼上即下載」，貼上後按「解析資訊」。日誌會列出網站提供的影片高度。
